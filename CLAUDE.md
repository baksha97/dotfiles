...
```bash
./main.sh setup [profile]        # Bootstrap the system (default: personal)
...
`main.sh` is the single entrypoint — it sources scripts from `meta/scripts/` rather than running them as subprocesses (`source`, not `exec`).
...
2. Homebrew packages — `meta/homebrew/Brewfile.<profile>` is the source of truth
...
| File | Purpose |
|------|---------|
| `meta/scripts/setup.sh` | Full bootstrap logic (Homebrew, SDKMAN!, stow, git profile, agent skills) |
